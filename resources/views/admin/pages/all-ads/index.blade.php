...
@section('crumb')
    <x-bread-crumb :breadcrumbs="[
        ['text'=>'All Ads','link'=>route('admin.cars.ads')],
        ]" :button="[]">
    </x-bread-crumb>
@endsection
...
{{--                                <th class="min-w-125px">Extentsion</th>--}}
                                ...
                                <th class="min-w-125px">Expire At</th>
...
{{--                                    @dd($car?->user)--}}
                                    <tr>
                                        <!--begin::Email=-->
                                        <td>
                                            {{ $car?->user?->name ?? $car->showroom?->showroom_name }}
                                        </td>
                                        <td>
                                            {{ $car->getUserType() }}
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <label class="form-check form-switch form-check-custom form-check-solid">
                                                <input type="checkbox"
                                                       @if($car->model_name == 'App\Models\Showroom' && $car->showroom->expired_date < now())
                                                            onchange="showAlert('{{route('admin.car.approve',$car->id)}}')"
                                                       @else
                                                           onchange="changeStatus('{{route('admin.car.approve',$car->id)}}')"
                                                       @endif
                                                       class="form-check-input" name="is_approved" @if ($car->is_approved) checked @endif>
                                            </label>
                                        </td>
